...
class RAG:
        ...
        if self.rerank_model in ["bge-reranker-base"]:
            ...
            for pair in text_pair:
                if isinstance(pair, (list, tuple)) and len(pair) >= 2:
                    # 确保每对文本都是字符串
                    processed_pair = [str(pair[0]) if pair[0] is not None else "", 
                                     str(pair[1]) if pair[1] is not None else ""]
                    processed_text_pair.append(processed_pair)
                else:
                    # 如果格式不对，跳过或使用默认值
                    processed_text_pair.append(["", ""])
            
            with torch.no_grad():
                inputs = EMBEDDING_MODEL_PARAMS["rerank_tokenizer"](
                    processed_text_pair, padding=True, truncation=True,
    ...
    def query_document(self, query: str, knowledge_id: int) -> List[str]:
        ...
        if self.use_rerank and len(sorted_content) > 0:
            text_pair = []
            for chunk_content in sorted_content:
                # 确保query和chunk_content都不是空的
                if query and chunk_content:
                    text_pair.append([query, chunk_content])
            
            if text_pair:  # 确保text_pair不为空
